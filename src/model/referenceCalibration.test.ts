...
import { scaleFromCalibrationPoints, snapImageToGrid } from "./referenceCalibration";
...
describe("scaleFromCalibrationPoints", () => {
  ...
  it("scales so one stitch and one row each come out a cell", () => {
    const fit = scaleFromCalibrationPoints(base, FOUR)!;
  ...
  it("gives the same scale whichever way the chart numbers its stitches", () => {
    ...
    const fit = scaleFromCalibrationPoints(base, mirrored)!;
    const straight = scaleFromCalibrationPoints(base, FOUR)!;
  ...
  it("averages a mark placed slightly off, rather than obeying it", () => {
    ...
    const slipped = FOUR.map((p) => (p.id === "b" ? { ...p, u: 0.436 } : p));
    const off = scaleFromCalibrationPoints(base, slipped)!;
    const exact = scaleFromCalibrationPoints(base, FOUR)!;
  ...
  it("ignores marks that have not been labelled yet", () => {
    ...
    const fit = scaleFromCalibrationPoints(base, partial)!;
    expect(fit.width).toBeCloseTo(CELL / 0.05, 8);
  });

  it("refuses a fit that no scale follows from", () => {
    const one = [FOUR[0]!];
    expect(scaleFromCalibrationPoints(base, one)).toBeNull();
    // Every mark naming the same stitch: no horizontal spread to fit.
    const sameStitch = FOUR.map((p) => ({ ...p, stitch: 36 }));
    expect(scaleFromCalibrationPoints(base, sameStitch)).toBeNull();
    // ...and the same for rows.
    const sameRow = FOUR.map((p) => ({ ...p, row: 1 }));
    expect(scaleFromCalibrationPoints(base, sameRow)).toBeNull();
  });
  ...
  it("refuses a scale the photo could not possibly carry", () => {
    ...
    expect(scaleFromCalibrationPoints(base, absurd)).toBeNull();

    // The same marks at a believable count are fine.
    const sane = [mark("a", 0, 0, 1, 1), mark("b", 1, 1, 20, 15)];
    expect(scaleFromCalibrationPoints(base, sane)).not.toBeNull();
  });
});
